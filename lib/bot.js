...
function createBot ({
  ...
  const sendWorker = co(function* (data) {
  ...
  const sends = managePersistentQueues({
    ...
    worker: strictlyQueued(sendWorker)
  })
  ...
  const doReceive = co(function* (user, wrapper) {
    ...
    yield bubble(handlers.receive, receiving)
    return receiving
  })

  const reliableReceive = co(function* ({ user, data }) {
    try {
  ...
  const receiveWorker = co(function* (data) {
    const ret = yield reliableReceive(data)
    try {
  ...
  const receives = managePersistentQueues({
    ...
    worker: strictlyQueued(receiveWorker)
  })
