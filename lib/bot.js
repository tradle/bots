 ...
 * @param  {Boolean}   [opts.autostart=true] if true, queued operations will commence immediately
 * @return {Object}
 */
function createBot ({ dir, send, seal, shouldRetry=DEFAULT_SHOULD_RETRY, autostart=true }) {
  const paths = {}
  if (dir) {
  ...
  const receives = managePersistentQueues({
    ...
    worker: co(function* tryReceive ({ user, data }) {
      try {
        ...
        forceLog(`Error receiving message due to error in strategy. Pausing receive for user ${user.id}`, err)
