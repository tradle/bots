...
const persistWithLowDB = require('./db/lowdb')
const NAMESPACES = {
  users: 'users',
  shared: 'shared',
...
function createBot ({
  ...
  for (let name in NAMESPACES) {
    dbs[name] = createDB({ dir: path.join(dir, NAMESPACES) })
    assertCompliantDB(dbs[name])
  }
  ...
  const users = manageUsers(dbs.users)
  const shared = manageShared(dbs.shared)
