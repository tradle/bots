let models
try {
  models = require('@tradle/models')
} catch (err) {
  /* eslint no-console: "off" */
  console.log('DEPRECATED, see https://github.com/tradle/bot-products')
  console.log('TO WORK WITH THIS STRATEGY, PLEASE PING mark@tradle.io FOR ACCESS TO https://github.com/tradle/models')
}
...
module.exports = function productsStrategy (bot, products=DEFAULT_PRODUCTS) {
  ...
  const oncreate = co(function* oncreate (user) {
    user.forms = {}
    user.applications = {}
    user.products = {}
  ...
  const removeReceiveHandler = bot.hooks.receive.on(onmessage)
