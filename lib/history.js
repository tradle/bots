...
// }
  ...
  function ensureHistoryFeed (userId) {
    ...
    if (!historyFeeds[userId]) {
      ...
      const feed = historyFeeds[userId] = promisifyAll(changesFeed(feedDB))
      feedReady[userId] = new Promise(resolve => feed.onready(() => resolve()))
    }
  ...
  const appendToHistory = co(function* ({ userId, item }) {
    ...
    yield feedReady[userId]
    item.metadata.index = feed.tentativeChange + 1
    typeforce(typeforce.String, userId)
