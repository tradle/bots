...
const { co, bigJsonParser } = require('./utils')

module.exports = function createApp ({ port, tradleServerURL, dbPath }) {
  // trim trailing slashes
  tradleServerURL = tradleServerURL.replace(/\/+$/, '')
  ...
  const bot = createBot({
     ...
     * @return {Promise}
     */
    send: function send (provider, user, payload) {
      const url = `${tradleServerURL}/${provider}/message`
