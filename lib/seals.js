...
module.exports = function manageSeals ({
  ...
  const hooks = {
    read: {
      on: addAndRemover(readHandlers)
    },
    newversion: {
      on: addAndRemover(newVersionHandlers)
    },
    wrote: {
      on: addAndRemover(wroteHandlers)
    }
  }
  ...
  const onread = co(function* (data) {
    try {
      yield bubble(readHandlers, data)
    } catch (err) {
      err.link = data.link
  ...
  const onnewversion = co(function* (data) {
    try {
      yield bubble(newVersionHandlers, data)
    } catch (err) {
  ...
  const onwrote = co(function* (data) {
    try {
      yield bubble(wroteHandlers, data)
    } catch (err) {
      err.link = data.link
