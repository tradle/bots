...
module.exports = function manageSeals ({
  ...
  const doSeal = co(function* (data) {
    ...
    debug(`sealed "${link}"`)
    try {
      yield hooks.fire('push', data)
    } catch (err) {
      err.link = link
      debug(`error in custom "push" handler`)
      hooks.emit('error', Errors.forAction(err, 'push'))
    }
  })
  ...
  const onread = co(function* (data) {
    ...
    try {
      yield hooks.fire('read', data)
    } catch (err) {
      err.link = data.link
      debug(`error in custom onRead handler`)
      hooks.emit('error', Errors.forAction(err, 'read'))
    }
  })
  ...
  const onnewversion = co(function* (data) {
    ...
    try {
      yield hooks.fire('newversion', data)
    } catch (err) {
      err.link = data.prevLink
      debug(`error in custom onRead handler`)
      hooks.emit('error', Errors.forAction(err, 'newversion'))
    }
  })
  ...
  const onwrote = co(function* (data) {
    ...
    try {
      yield hooks.fire('wrote', data)
    } catch (err) {
      err.link = data.link
      debug(`error in custom onWrote handler`)
      hooks.emit('error', Errors.forAction(err, 'wrote'))
    }
  })
