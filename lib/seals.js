...
module.exports = function manageSeals ({ dir, store, seal, shouldRetry=defaultShouldRetry, autostart=true }) {
  ...
  const queues = queuey({ db: levelup(paths.queue) })
  let queue
  ...
  const update = co(function* update (data) {
    const { link } = data
    const stored = yield store.get(link)
