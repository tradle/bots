...
module.exports = function manageSeals ({
  ...
  const queueDBPath = path.join(dir, prefix + '-queue')
  const queues = queuey(inMemory ? null : queueDBPath)
  ...
  const update = co(function* update (data) {
    ...
    try {
      ...
      debug(`received "readseal" event for "${link}" seal I did not request. Just saying.`)
      stored = {}
    }
