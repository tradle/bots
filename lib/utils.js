...
const { promisify, promisifyAll } = Promise
...
function interpretObjectWrapper (wrapper) {
  if (wrapper.metadata) return wrapper

  return {
    metadata: getMetadata(wrapper),
    object: wrapper.object
  }
}

function interpretMessageWrapper (wrapper) {
  if (wrapper.metadata) return wrapper
